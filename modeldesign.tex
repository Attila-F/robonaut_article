...
In the first part of the article we will demonstrate how the focus of the software development has shifted from the actual coding to a more visual and mathematical representation of the system, and how to harness its traits to \emph{quickly} develop a \emph{reliable} system. In the second part it will be described how to use the \verb!MATLAB! \verb!Coder! auto code generation tool and how to integrate the source with (the?) \verb!FreeRTOS! (hard real-time operating system?) and deploy it on the target hardware, the \verb!STM32F4-Discovery! developer board.
